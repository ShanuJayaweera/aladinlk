...
try{
    ...
    if(isset($_POST['heavy'])){
        ...
        //text box validation
        $text_box=array('title','description','brand');
        $errors=array_merge($errors,check_req_fields($text_box));


        //length validation
        $max_len_fields=array('title'=>100,'description'=>5000,'model_year'=>4,'brand'=>50);
        ...
        if(data_availability('heavy','ad_id','ad_id',$ad_id)==false){
            $errors[]="Wrong Ad ID";
        }
        ...
        if (empty($errors)){
            ...
            $heavy="update heavy set title='".$title."', vehicle_condition='".$condition."', model_year='".$model_year."', description='".$description."', price='".$price."',brand='".$brand."',image_path='".$image_path."' where ad_id=".$ad_id.";";

            mysqli_query(DBConnection(), $heavy);
            mysqli_close(DBConnection());
