...
try{
    ...
    if(isset($_POST['push'])){
        ...
        if(data_availability('push_auto','ad_id','ad_id',$ad_id)==false){
            $errors[]="Wrong Ad ID";
        }
        ...
        if(empty($errors)){
            ...
            $push_auto="update push_auto set title='".$title."', vehicle_condition='".$condition."', description='".$description."', price='".$price."',image_path='".$image_path."' where ad_id=".$ad_id.";";

            $main_img=main_img($_SESSION['ad_id']);
            $image_path=get_images($_SESSION['ad_id']);

            mysqli_query(DBConnection(), $push_auto);
            mysqli_close(DBConnection());
