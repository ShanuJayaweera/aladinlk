...
//check is advertisement id available in database
    ...
    if(data_availability('default1','ad_id','ad_id',$ad_id)==false){
        $errors[]="Wrong Ad ID";
    }
    ...
    if(empty($errors)){
        ...
        $image_path=get_images($_SESSION['ad_id']);
            ...
            $default="update default1 set item_condition='".$condition."', title='".$title."', 
            description='".$description."', price='".$price."',image_path='".$image_path."' where ad_id=".$ad_id.";";

        mysqli_query(DBConnection(), $default);
        mysqli_close(DBConnection());
